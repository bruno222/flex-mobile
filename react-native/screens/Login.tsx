...
export const Login = view(() => {
   ...
   *
   * Example of "url":
   *   https://serverless-sso-6931-dev.twil.io/sso/login?id=ONELOGIN_506201c0-f7e9-4c21-99bb-5e524e042d13&RelayState=AAAAAS5GQVNfUzNf
   *
   *   outcome is flexTokenStore.runtimeDomain = https://serverless-sso-6931-dev.twil.io
   */
  // const captureDomain = (url: string) => {
  //   const FIND_PATH = '/sso/login';
  //   if (!url.includes(FIND_PATH)) {
  //     return;
  //   }

  //   flexTokenStore.runtimeDomain = url.substring(0, url.indexOf(FIND_PATH));
  //   console.log('@@ runtimeDomain', flexTokenStore.runtimeDomain); // https://serverless-sso-6931-dev.twil.io
  //   return;
  // };
   ...
   */
  const interceptor = async ({ url }: any) => {
    // captureDomain(url);
    ...
    webview.stopLoading();
    await flexTokenStore.set(Token);
    return;
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <WebView
        userAgent="flex-mobile-react-native"
        ref={(ref) => (webview = ref)}
        source={{ uri: FLEX_SSO_URL }}
